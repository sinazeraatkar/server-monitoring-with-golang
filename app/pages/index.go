...
func DashboardPage(ctx *context.Context) (types.Panel, error) {
	...
	DatabaseConnection, err = gorm.Open(postgres.Open(fmt.Sprintf("host=postgres port=5432 user=%s dbname=%s password=%s TimeZone=Asia/Tehran", SystemConfig.DatabaseConfig.DatabaseUsername, SystemConfig.DatabaseConfig.DatabaseName, SystemConfig.DatabaseConfig.DatabasePassword)), &gorm.Config{})
